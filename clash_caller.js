...
var my_clan_name = process.env.CLAN_NAME; // Your clan name
var war_call_timer = 3; // Timer for calls. How long until a call expires.
var user_name = "";
var user_id = "";
var caller_code = "";
...
exports.add_admin = function(gm_id) {  
  conn.query('SELECT * FROM `admins` where gmid = ?', [gm_id], function(err, res, fld){
      if (res.length == 0) {
		  conn.query('INSERT INTO `admins` (gmid) VALUES (?)', [gm_id]);
		  gm_text_ = "Added new admin " + gm_id;
		  gm_text_.save_log();
		  gm_text_.post_text();
		  console.log(gm_text_);
	  } else {
		  gm_text_ = gm_id + " is already an admin";
		  gm_text_.post_text();	  
	  }
  });  
}

exports.remove_admin = function(gm_id) {  
  conn.query('SELECT * FROM `admins` where gmid = ?', [gm_id], function(err, res, fld){
	  if (res.length > 0) {
		  conn.query('DELETE FROM `admins` where gmid = ?', [gm_id]);
		  gm_text_ = "Removed " + gm_id + " as admin";
		  gm_text_.save_log();
		  gm_text_.post_text();
		  console.log(gm_text_);  
	  } else {
		  gm_text_ = gm_id + " is not an admin";
		  gm_text_.post_text();	  	  
	  }	  
  });  
}

function is_admin_db(gm_id) {  
  result = false;
  conn.query('SELECT * FROM `admins` where gmid = ?', [gm_id], function(err, res, fld){
    result = (res.length > 0);
  });  
...
exports.is_user_admin = function(user_id) {
  ...
  return (in_array(ad_, user_id) || is_admin_db(user_id))
}
