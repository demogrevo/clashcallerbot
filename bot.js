    ...
    async.waterfall([
      ...
      function(cc_code, callback) {
        ...
        var regex_ = {
		  ...
		  add_admin: /^\/add admin (\d+)\s*$/i,
		  remove_admin: /^\/remove admin (\d+)\s*$/i,
          prof_id: /^\/me\s*$/i,
          cc_url: /^\/cc\s*$/i,
          set_code: /^\/set cc\s+(.*)$/i,
          get_code: /^\/code\s*$/i,
          ...
          if (regex_[index].test(input)) {
            ...
            switch (index) {
              ...
              case 'add_admin':
                do_it = cc_.is_user_admin(user_id);
                if (do_it) {                  
                  gm_id = input.match(regex_[index])[1];
                  cc_.add_admin(gm_id);
                } else {
                  message_ = "Only admins can add new admin, @" + user_name;
                }
                break;				
              case 'remove_admin':
                do_it = cc_.is_user_admin(user_id);
                if (do_it) {                  
                  gm_id = input.match(regex_[index])[1];
                  cc_.remove_admin(gm_id);
                } else {
                  message_ = "Only admins can remove other admin, @" + user_name;
                }
                break;								
              case 'help':
                ...
                if (admin_help == '-a') {
                  message_ += 
					...
					"/add admin gm_id - Add a new admin\n" +
					"/remove admin gm_id - Revoke admin privilege\n";
                }
            }
          }
        }
